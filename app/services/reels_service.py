...
class ReelsService:
    ...
    async def _analyze_video_path(self, reel_in: ReelIn, video_path: Path) -> Tuple[Optional[FrameAnalysis], int, int]:
        video_file = None
        try:
    ...
    async def analyze_reel_video(self, reel_in: ReelIn) -> FrameAnalysis:
        ...
        try:
            ...
            if isinstance(video_result, Exception):
                logger.error(f"Video analysis path failed: {video_result}", exc_info=video_result)
                raise RuntimeError("Video analysis failed, cannot proceed.") from video_result

            analysis_result, v_prompt, v_resp = video_result
            prompt_token_total += v_prompt
            response_token_total += v_resp
            total_cost += cost_service.calculate_cost(self.llm_config.get("model"), v_prompt, v_resp)
